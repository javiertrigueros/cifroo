...
(function () {
    ...
    var defaultsStrings = {
        ...
        underline: "Underline <u> Ctrl+S",
        underlinesample: "Sample underline",
    ...
    */
    ...
    function UIManager(postfix, panels, undoManager, previewManager, commandManager, helpOptions, getString, getKey) {
        ...
        var inputBox = panels.input,
            ...
            //esto para pasar bien undoManager 
            //tiene algun tipo de error
            //undoManager = inputBox.session.getUndoManager(); 
        ...
        var that = this;
        makeSpritedButtonRow(inputBox.session.getUndoManager());
        ...
        addKeyCmd(['bold', 'italic', 'underline', 'link', 'quote', 'code', 'image', 'olist', 'ulist', 'heading', 'hr']);
        ...
        */
        ...
        function makeSpritedButtonRow(manager) {
            ...
            buttons.undo = makeButton("wmd-undo-button", getString("undo"), "0px", null);
            buttons.undo.execute = function () { 
            ...
            buttons.redo = makeButton("wmd-redo-button", redoTitle, "-10px", null);
            buttons.redo.execute = function () {
                if (manager) manager.redo(); 
            };
            makeSpacer(1);

            buttons.heading = makeButton("wmd-heading-button", getString("heading"), "-30px", bindCommand("doHeading"));
            buttons.bold = makeButton("wmd-bold-button", getString("bold"), "-40px", bindCommand("doBold"));
            buttons.italic = makeButton("wmd-italic-button", getString("italic"), "-50px", bindCommand("doItalic"));
            buttons.underline = makeButton("wmd-underline-button", getString("underline"), "-60px", bindCommand("doUnderline"));
            makeSpacer(1);
            
            buttons.ulist = makeButton("wmd-ulist-button", getString("ulist"), "-80px", bindCommand(function (chunk, postProcessing) {
                this.doList(chunk, postProcessing, false);
            }));
            buttons.olist = makeButton("wmd-olist-button", getString("olist"), "-90px", bindCommand(function (chunk, postProcessing) {
                this.doList(chunk, postProcessing, true);
            }));
            buttons.quote = makeButton("wmd-quote-button", getString("quote"), "-100px", bindCommand("doBlockquote"));
            buttons.hr = makeButton("wmd-hr-button", getString("hr"), "-110px", bindCommand("doHorizontalRule"));
            makeSpacer(1);
            
            buttons.link = makeButton("wmd-link-button", getString("link"), "-130px", bindCommand(function (chunk, postProcessing) {
            ...
            //buttons.code = makeButton("wmd-code-button", getString("code"), "-150px", bindCommand("doCode"));
            makeSpacer(2);
    ...
    commandProto.doUnderline = function (chunk, postProcessing) {
        return this.doUnder(chunk, this.getString("underlinesample"));
    };
    
    commandProto.doUnder = function (chunk,  insertText) {
